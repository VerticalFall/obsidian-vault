...
name: health-monitor
description: >
  L1 系统健康体检：零 LLM 零成本检查四源产出、X 推文计数、路由日志、选题池存量（含表格质量）、
  翻译成功率、待验证问题，输出 _系统健康/YYYY-MM-DD.md 日报 + 滚动 _告警摘要.md（仅 🔴/🟡）+ 待验证问题.md。
...
| 4 | 选题池存量 + 表格质量 | `_选题池.md` 🔥+🌿 数量是否健康；表格是否有模板残留行/重复行（🟡） |
| 5 | 翻译成功率 | X-Tweets 翻译是否正常 |
| 6 | 待验证问题 | `待验证问题.md` 是否有未处理条目（非空 → 🟡，提醒闭环；不进告警摘要） |

输出：`_系统健康/YYYY-MM-DD.md` + 滚动 `_告警摘要.md`（**仅记 🔴/🟡**）+ `待验证问题.md`（追加式）。
...
- [ ] 日报已生成；🔴/🟡 已进入 `_告警摘要.md`
- [ ] 只有**需人工核实**的事项才追加到 `待验证问题.md`（同一天同一检查项不重复）
