...
# 技能库索引（11 个 Claude 技能）

> Claude Code 通过 vault 根目录 `.claude/skills` 目录联接（junction）加载本目录的 11 个技能。
...
| [[daily-router/SKILL.md\|daily-router]] | daily_router | `run_daily_router.py`（DeepSeek 三层漏斗 → 路由日志 + 选题池） |
| [[daily-digest/SKILL.md\|daily-digest]] | 每日精选 | `render_daily_digest.py`（四源 + 路由 → 一份当日精选） |
| [[weekly-distill/SKILL.md\|weekly-distill]] | weekly_distill | `run_weekly_distill.py`（周度信号蒸馏） |
| [[health-monitor/SKILL.md\|health-monitor]] | 日常监控 | `check_health.py`（L1 零 LLM 体检，9 项） |
