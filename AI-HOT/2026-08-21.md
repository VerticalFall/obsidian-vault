...
1. **阿里发布 Qwen-UI-Agent，主打让模型真正“会用”每一块屏幕** — IT之家（RSS）
   ...
   [查看原文](https://www.ithome.com/0/992/239.htm)
...
2. **Hugging Face 发布 LFM2.5 系列 DSpark 草稿模型，推理速度最高提升 3.18 倍** — Hugging Face：Blog（RSS）
   ...
   [查看原文](https://huggingface.co/blog/LiquidAI/lfm25-dspark)
...
3. **Mistral 推出 Agentic Search：多步检索提升 AI 系统复杂文档查询准确率** — Mistral AI：News（网页）
   ...
   [查看原文](https://mistral.ai/news/agentic-search)
...
4. **AlloyDB ScaNN 如何将向量搜索扩展到 100 亿向量** — Google Cloud：Databases（RSS）
   ...
   [查看原文](https://cloud.google.com/blog/products/databases/alloydb-scann-index-four-level-tree-improves-vector-search)
...
5. **Claude Platform 正式上线 Computer Use、Skills API 与 Files API，新增浏览器操作工具** — Claude：Blog（网页）
   ...
   [查看原文](https://claude.com/blog/computer-use-skills-api-files-api)
...
6. **Anthropic 如何开展 AI 教学** — Claude：Blog（网页）
   ...
   [查看原文](https://claude.com/blog/anthropics-approach-to-teaching-and-learning-ai)
...
7. **Mooncake 如何为 Miles 强化学习系统实现高效批量 Rollout 数据传输** — LMSYS：Blog（Chatbot Arena 团队）
   ...
   [查看原文](https://www.lmsys.org/blog/2026-08-20-miles-mooncake-rollout-data-transfer)
...
8. **Claude Code v2.1.238 发布：新增 readline 键位、插件市场 headersHelper 与多项 Remote Control 修复** — Claude Code：GitHub Releases（RSS）
   ...
   [查看原文](https://github.com/anthropics/claude-code/releases/tag/v2.1.238)
...
9. **Claude Code v2.1.237 发布：修复 LLM 网关提示词缓存，新增“简洁”输出风格** — Claude Code：GitHub Releases（RSS）
   ...
   [查看原文](https://github.com/anthropics/claude-code/releases/tag/v2.1.237)
...
10. **LangSmith 预览构建：如何在合并前测试智能体变更** — LangChain：Blog（RSS）
   ...
   [查看原文](https://www.langchain.com/blog/langsmith-preview-builds-test-agent-changes-before-production)
...
11. **消息称 OpenAI 首席财务官告知员工：公司最迟将于 2027 年上市** — IT之家（RSS）
   ...
   [查看原文](https://www.ithome.com/0/991/886.htm)
...
12. **数据受限下的多语言知识迁移：Apple 提出基于词汇干预的新方法** — Apple Machine Learning Research（RSS）
   ...
   [查看原文](https://machinelearning.apple.com/research/multilingual-knowledge-transfer-lexical-interventions)
...
13. **数据约束下的混合预训练缩放定律** — Apple Machine Learning Research（RSS）
   ...
   [查看原文](https://machinelearning.apple.com/research/scaling-laws-mixture-pretraining)
...
14. **Claude Code 初创公司指南：五大规则与创始人洞见** — Claude：Blog（网页）
   ...
   [查看原文](https://claude.com/blog/claude-code-guide-for-startups)
...
15. **OpenAI 推出 AI Futures 博客，探讨自由社会如何应对变革性 AI** — OpenAI：官网动态（RSS · 排除企业/客户案例）
   ...
   [查看原文](https://openai.com/index/introducing-ai-futures)
...
16. **Leopold 的愚蠢：一个年轻人如何象征一个时代** — Gary Marcus：The Road to AI We Can Trust（RSS）
   ...
   [查看原文](https://garymarcus.substack.com/p/leopolds-folly)
...
17. **共和党因与大型科技公司关系陷入恐慌** — Gary Marcus：The Road to AI We Can Trust（RSS）
   ...
   [查看原文](https://garymarcus.substack.com/p/breaking-the-republican-party-is)
