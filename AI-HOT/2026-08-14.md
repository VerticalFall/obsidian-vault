...
1. **小红书开源连续自回归语音合成模型 dots.tts：打造可持续扩展的 TTS 基座** — 公众号：小红书技术（dots.llm）
   小红书 dots 团队开源 20 亿参数全连续端到端自回归语音合成模型 dots.tts，在 Seed-TTS-Eval 三个子集上取得最佳平均内容准确度和平均说话人相似度。
   [查看原文](https://mp.weixin.qq.com/s?__biz=Mzg4OTc2MzczNg%3D%3D&mid=2247496062&idx=1&sn=d4c48926c5d7607f129dfea03699a6c0)

2. **Google DeepMind 推出 Gemini 3.7 Flash：面向编程与智能体的最强工作模型** — Google DeepMind：Blog（RSS）
   Google DeepMind 发布 Gemini 3.7 Flash，距 3.6 Flash 仅三周，主打编程与智能体任务，输入/输出价格分别为每百万 token $0.75 和 $3.75，为原 3.6 Flash 的一半。
   [查看原文](https://deepmind.google/blog/introducing-gemini-3-7-flash)

3. **MiniMax Music 3.0 发布：新一代开源权重、生产级全能音乐模型** — MiniMax：Blog（网页）
   MiniMax 推出 Music 3.0，新一代音乐生成模型，可根据创意概念和可选歌词一次性完成整首歌的作曲、编曲、演奏与制作，最长支持五分钟。
   [查看原文](https://www.minimax.io/blog/minimax-music-3-0-next-generation-open-weights-production-ready-versatile-music-model)

## 📦 产品发布/更新

4. **DeepSeek Harness v0.1 开发者预览版发布** — X：DeepSeek (@deepseek_ai)
   ...
   [查看原文](https://x.com/deepseek_ai/status/2087887408440164663)

5. **Cursor 推出 builds：云智能体启动速度提升至 3 倍** — Cursor Blog
   ...
   [查看原文](https://cursor.com/blog/builds)

6. **WorkBuddy上线远程控制，国内也有了最丝滑的Agent工作方式** — 公众号：数字生命卡兹克
   ...
   [查看原文](https://mp.weixin.qq.com/s?__biz=MzIyMzA5NjEyMA%3D%3D&mid=2647685179&idx=1&sn=4cbee32b677b57a9dc6ec2a79d6f72c7)

7. **Google Sheets 推出 Sheets canvas：用 Gemini 将表格数据变为交互式迷你应用** — Google Blog：AI（RSS）
   Google Sheets 发布新功能 Sheets canvas，基于 Gemini 构建，用户只需用自然语言提示词即可将表格数据转化为交互式仪表盘、学习追踪器、座位表等“迷你应用”。
   [查看原文](https://blog.google/products-and-platforms/products/workspace/sheets-canvas-for-google-sheets-spreadsheets)

8. **Google 发布开源 C++ 库 Credentio，用于 C2PA 内容凭证验证** — Google Developers Blog（RSS）
...
9. **BigQuery Graph 新增 measures 支持，为智能体工作负载提供可信关系推理** — Google Cloud：Databases（RSS）
   Google Cloud 在 BigQuery Graph（预览版）中引入 measures 支持，将治理指标与关系映射统一，使 AI 智能体能在图结构上基于精确指标进行推理，解决传统表格无法追踪多跳业务关系导致错误决策的问题。
   [查看原文](https://cloud.google.com/blog/products/data-analytics/bigquery-graphs-with-measures-for-trusted-agentic-workloads)

10. **OpenAI 预览 Ultrafast 模式：GPT-5.6 Sol 速度提升最高 14 倍** — OpenAI：官网动态（RSS · 排除企业/客户案例）
   OpenAI 推出新的 API 服务层级 Ultrafast，由 Cerebras 提供算力，运行 GPT-5.6 Sol 的速度最高提升 14 倍，输出速率可达每秒 750 tokens。该模式目前处于预览阶段。
   [查看原文](https://openai.com/index/previewing-ultrafast)

11. **Claude Code v2.1.232 发布：默认启用 Subagent forking，新增 GitLab 支持与多项安全修复** — Claude Code：GitHub Releases（RSS）
   ...
   [查看原文](https://github.com/anthropics/claude-code/releases/tag/v2.1.232)

## 🏭 行业动态

12. **Cursor 获得 AIUC-1 认证，通过智能体安全与可靠性独立审查** — Cursor Blog
   ...
   [查看原文](https://cursor.com/blog/aiuc-1)

13. **Firetiger 团队加入 Cursor** — Cursor Blog
...
14. **OpenAI 任命 Dali Rajic 为首席营收官** — OpenAI：官网动态（RSS · 排除企业/客户案例）
   OpenAI 任命 Dali Rajic 为首席营收官，负责领导其全球营收组织，帮助企业充分实现 AI 的价值。
   [查看原文](https://openai.com/index/dali-rajic-chief-revenue-officer)

## 📄 论文研究

15. **新兴多智能体系统的模式与问题** — Anthropic：Research（发表成果 · 网页）
   ...
   [查看原文](https://www.anthropic.com/research/multiagent-systems)

16. **当“遗忘”无需成本：利用低影响力数据点降低机器学习计算开销** — Apple Machine Learning Research（RSS）
   ...
   [查看原文](https://machinelearning.apple.com/research/unlearning-low-influence-points)

## 💡 技巧与观点

17. **GPT-5.6 构建者指南：如何以更低成本实现前沿智能体性能** — OpenAI：官网动态（RSS · 排除企业/客户案例）
   ...
   [查看原文](https://openai.com/index/builders-guide-to-gpt-5-6)

18. **Claude 接管应用日常维护：388 个 PR 的实践** — X：Boris Cherny (@bcherny)
   ...
   [查看原文](https://x.com/bcherny/status/2088014489438621990)

19. **Strands Robots 如何用 Hugging Face Storage Buckets 实现记录、训练与部署一体化** — Hugging Face：Blog（RSS）
   AWS 开源的 Strands Robots（Apache 2.0）通过单一智能体循环，将机器人演示记录、基于 Hub 数据流的策略训练及硬件部署整合在一起，全程保持 LeRobot 磁盘格式不变。
   [查看原文](https://huggingface.co/blog/amazon/strands-lerobot-streaming-data-loop)

20. **Anthropic 如何在 Slack 中用 Claude Tag 部署自助式数据分析智能体** — Claude：Blog（网页）
   ...
   [查看原文](https://claude.com/blog/self-service-data-analytics-in-slack-how-anthropic-deploys-claude-tag-for-ad-hoc-questions)

21. **JetBrains CTO 谈如何评估并部署 Claude Fable 5：私有仓库评测、效率提升与安全策略** — Claude：Blog（网页）
   ...
   [查看原文](https://claude.com/blog/how-jetbrains-evaluates-and-deploys-claude-fable-5)

22. **OpenAI 黑客事件与意图之问：智能体逃逸沙箱窃取密码，控制才是关键** — Tomer Tunguz 博客（VC 分析）
   ...
   [查看原文](https://www.tomtunguz.com/openai-hack-ai-intent)

23. **GitHub Secure Open Source Fund 第四期 50 个开源项目如何提升 AI 时代安全性** — GitHub Blog
   ...
   [查看原文](https://github.blog/open-source/maintainers/what-50-open-source-projects-taught-us-about-security-in-the-ai-era)
