...
2. **OpenRouter 推出 Activity 仪表盘与 Analytics API：按智能体、模型、请求追踪 AI 使用成本** — OpenRouter：Announcements（RSS）
   ...
   [查看原文](https://openrouter.ai/blog/announcements/activity-dashboard)
...
3. **OpenRouter 图像生成 API：代码优先的接入教程** — OpenRouter：Announcements（RSS）
   ...
   [查看原文](https://openrouter.ai/blog/tutorials/image-generation)
...
4. **AgentCore Payments 中间件为 LangChain 智能体提供 API 支付能力** — LangChain：Blog（RSS）
   ...
   [查看原文](https://www.langchain.com/blog/langchain-agentcore-payments)
...
5. **Claude Code v2.1.234 发布：新增项目目录名变量与 GitLab MR 徽章，修复多项安全与稳定性问题** — Claude Code：GitHub Releases（RSS）
   ...
   [查看原文](https://github.com/anthropics/claude-code/releases/tag/v2.1.234)
...
6. **NVIDIA 与 SB Energy 合作锁定俄亥俄州 PORTS-Pike 园区电力容量，OpenAI 将入驻** — NVIDIA Blog（RSS）
   ...
   [查看原文](https://blogs.nvidia.com/blog/securing-the-infrastructure-of-intelligence)
...
7. **A 股迎来“人形机器人第一股”，宇树科技官宣 8 月 19 日科创板上市** — IT之家（RSS）
   ...
   [查看原文](https://www.ithome.com/0/990/812.htm)
...
8. **404 Media 追踪珍本图书流向：亚马逊批量购书扫描用于 AI 训练后销毁** — Hacker News 热门（buzzing.cc 中文翻译）
   ...
   [查看原文](https://www.404media.co/we-tracked-a-shipment-of-rare-books-it-ended-at-an-amazon-ai-training-facility)
...
9. **OpenAI 为 14 个独立项目提供资助，推动智能时代经济机遇与韧性研究** — OpenAI：官网动态（RSS · 排除企业/客户案例）
   ...
   [查看原文](https://openai.com/index/new-policy-ideas-for-the-intelligence-age)
...
10. **PhotoScan：用智能手机照片估算胰岛素抵抗，精度接近DXA** — Google Research：Blog（网页）
   ...
   [查看原文](https://research.google/blog/seeing-beyond-bmi-estimating-cardiometabolic-risk-with-smartphone-imagery)
...
11. **OpenAI 如何用前沿智能加固自身防御：The Defender’s Window** — OpenAI：官网动态（RSS · 排除企业/客户案例）
   ...
   [查看原文](https://openai.com/index/the-defenders-window)
...
12. **如何禁用或避免侵入式 AI：一份覆盖 Windows、Chrome、Edge、Firefox 及主流应用的实用指南** — Hacker News 热门（buzzing.cc 中文翻译）
   ...
   [查看原文](https://www.librarian.net/notoai)
...
14. **开源模型生态的未来：Nvidia 押注“教所有人炼 token”** — Nathan Lambert：Interconnects（RSS）
   ...
   [查看原文](https://www.interconnects.ai/p/teaching-everyone-to-fish-for-tokens)
...
15. **同一集群利用率提升 33 个百分点：改变的是分配顺序** — Hugging Face：Blog（RSS）
   ...
   [查看原文](https://huggingface.co/blog/Dharma-AI/gpu-management-pt2)
...
16. **ABC Legal 如何借助 Claude Managed Agents 让每位员工成为构建者** — Claude：Blog（网页）
   ...
   [查看原文](https://claude.com/blog/how-abc-legal-turned-every-employee-into-a-builder-with-claude-managed-agents)
...
17. **当模型持续学习：测试时训练如何改变 AI 的记忆与成本** — Tomer Tunguz 博客（VC 分析）
   ...
   [查看原文](https://www.tomtunguz.com/test-time-training-impact)
...
18. **SGLang 重构 CUDA Graph 支持，Breakable CUDA Graph 成 prefill 默认方案** — LMSYS：Blog（Chatbot Arena 团队）
   ...
   [查看原文](https://www.lmsys.org/blog/2026-08-17-advanced-cuda-graph)
