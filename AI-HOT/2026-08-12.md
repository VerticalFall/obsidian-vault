...
1. **NVIDIA 推出 Nemotron 3.5 Lightning，加速本地智能体任务** — NVIDIA Blog（RSS）
   ...
   [查看原文](https://blogs.nvidia.com/blog/local-ai-open-source-models-agents-nemotron)

2. **SGLang 宣布 Day-0 支持 NVIDIA Nemotron 3.5 Lightning** — LMSYS：Blog（Chatbot Arena 团队）
   ...
   [查看原文](https://www.lmsys.org/blog/2026-08-11-nemotron-3-5-lightning)

3. **Ling-3.0-tiny 正式开源：1.3B 激活参数如何进入真实任务** — 公众号：蚂蚁百灵（Ling）
   蚂蚁百灵开源 Ling-3.0-tiny，一款总参数 7.9B、推理时仅激活 1.3B 参数的原生混合推理模型，同步提供 BF16、FP8 和 INT4 三个版本。
   [查看原文](https://mp.weixin.qq.com/s?__biz=MzkyODk2MDQwNw%3D%3D&mid=2247487491&idx=1&sn=fcb14aceb054f9a24e22525d3dae6fa0)

## 📦 产品发布/更新

4. **Runway Seedance 2.5 上线，支持50角色参考** — X：Runway (@runwayml)
   完整阵容，完整曲目，一次生成一个。 Seedance 2.5 已在 Runway 上线，支持 50 个独特角色参考，以及最长 30 秒、与音乐同步的片段。点击下方链接即可开始使用。
   [查看原文](https://x.com/runwayml/status/2087251184658657346)

5. **Gemini 助力 Database Migration Service 加速 PostgreSQL 迁移** — Google Cloud：Databases（RSS）
   ...
   [查看原文](https://cloud.google.com/blog/products/databases/accelerate-postgresql-migrations-with-gemini-in-dms)

6. **ZCode全面升级：Goal、Subagents、Remote Control与闲时任务四大功能上线** — 公众号：智谱（GLM）
   ...
   [查看原文](https://mp.weixin.qq.com/s?__biz=MzkyMzI3NzQ0Mg%3D%3D&mid=2247494052&idx=1&sn=ee3ab3d0f4550e9120927c53a27522c9)

7. **ChatGPT 桌面端支持导入其他智能体工作数据** — X：OpenAI Developers (@OpenAIDevs)
   你现在可以将其他智能体的工作内容与 ChatGPT Work 和 Codex 保持同步。 可导入项目、聊天记录、技能和插件，查看导入历史，并可在设置中选择开启自动更新。 现已在 ChatGPT 桌面应用中提供。
   [查看原文](https://x.com/OpenAIDevs/status/2087242829076791392)

8. **Databricks 开源 Metals v2：面向数百万行代码库的 Java 和 Scala 语言服务器** — Databricks：Blog（RSS）
   ...
   [查看原文](https://www.databricks.com/blog/open-sourcing-metals-v2-databricks-java-and-scala-language-server-multi-million-line-codebases)
...
9. **研究人员发现可读取ChatGPT等模型加密推理过程的API漏洞** — The Decoder：AI News（RSS）
   ...
   [查看原文](https://the-decoder.com/but-marinade-and-leaked-passwords-are-what-researchers-found-in-chatgpts-hidden-reasoning)

10. **消息称 Anthropic 最快今年 9 月上市，向投资者淡化 AI 模型竞争等挑战** — IT之家（RSS）
   ...
   [查看原文](https://www.ithome.com/0/988/239.htm)

11. **Gemini月活破10亿，成谷歌增长最快产品** — X：Sundar Pichai (@sundarpichai)
   ...
   [查看原文](https://x.com/sundarpichai/status/2087222656819241292)

12. **消息称英伟达开发万亿参数开源 AI 模型 Nemotron 4，目标挑战全球顶级** — IT之家（RSS）
   ...
   [查看原文](https://www.ithome.com/0/988/524.htm)

13. **NVIDIA 为何需要新供电架构以扩展 AI 算力性能** — NVIDIA Blog（RSS）
   ...
   [查看原文](https://blogs.nvidia.com/blog/800-vdc-power-architecture-ai-factory)

14. **英伟达循环融资达到新高度，黄仁勋是否过度出牌？** — Gary Marcus：The Road to AI We Can Trust（RSS）
   ...
   [查看原文](https://garymarcus.substack.com/p/breaking-circular-financing-reaches)

15. **Electric 加入 Databricks，将 WASM Postgres 引入 AI 智能体沙箱** — Databricks：Blog（RSS）
   ...
   [查看原文](https://www.databricks.com/blog/electric-joins-databricks-bring-wasm-postgres-ai-agent-sandboxes)

## 📄 论文研究

16. **Apple Silicon 与 macOS 虚拟机：借助 Llama.cpp 实现 11–16 倍的 LLM 推理加速** — Hacker News 热门（buzzing.cc 中文翻译）
   ...
   [查看原文](https://github.com/trycua/cua/blob/main/blog/gpu-passthrough-macos-vms.md)

17. **统一 Radix 缓存：为混合模型前缀缓存构建单一树结构** — LMSYS：Blog（Chatbot Arena 团队）
   LMSYS 团队提出 Unified Radix Cache，用单一 token 键控 radix 拓扑统一管理混合模型的 FULL、SWA 和 MAMBA 组件缓存，各组件独立执行路径、滑动窗口和检查点复用语义。
   [查看原文](https://www.lmsys.org/blog/2026-08-11-unified-radix-cache)

18. **AMIE 研究医疗 AI 系统首次展示实时临床视频问诊能力** — Google Blog：AI（RSS）
   ...
   [查看原文](https://blog.google/innovation-and-ai/models-and-research/google-research/amie-video-consultations)

## 💡 技巧与观点

19. **OpenAI 用 Astra 模型攻克 10 道数学难题，数学家既兴奋又担忧** — The Verge：AI（RSS）
   OpenAI 宣布其未发布的 Astra 模型解决了 10 道长期悬而未决的数学难题，涵盖球体堆积、纠错码、非 sofic 群存在性等领域，并发布超 250 页论文及 Lean 验证结果。
   [查看原文](https://www.theverge.com/ai-artificial-intelligence/977273/the-ai-takeover-of-mathematics-has-begun)

20. **用 ComfyUI API 实现 MiniMax-H3 多模态视频与音频生成流水线** — MarkTechPost（RSS）
   ...
   [查看原文](https://www.marktechpost.com/2026/08/10/implementing-a-minimax-h3-multimodal-video-and-audio-generation-pipeline-with-comfyui-apis)

21. **将 GitHub Copilot 置于中间人（MitM）代理之后后，我学到了什么** — Hacker News 热门（buzzing.cc 中文翻译）
   ...
   [查看原文](https://www.lighthousenewsletter.com/p/i-put-github-copilot-behind-a-mitm)

22. **编写智能体时，哪种编程语言最合适？** — Hacker News 热门（buzzing.cc 中文翻译）
   ...
   [查看原文](http://danluu.com/pl-tokens)

23. **微信小微AI帮写与AI点评内测：朋友圈最后一点人味正在消失** — 公众号：数字生命卡兹克
   ...
   [查看原文](https://mp.weixin.qq.com/s?__biz=MzIyMzA5NjEyMA%3D%3D&mid=2647685081&idx=1&sn=13a05578a3959c047209c8e74b56fa48)

24. **Ryan Greenblatt：人类级AI或于2032年前通过递归自我改进催生失控超级智能** — Dwarkesh Patel：Podcast & Blog（RSS）
   ...
   [查看原文](https://www.dwarkesh.com/p/ryan-greenblatt)

25. **每个类别都有赢家：AI 时代 SaaS 龙头的估值溢价** — Tomer Tunguz 博客（VC 分析）
   SaaS 估值整体承压，但每个细分赛道都跑出了 AI 龙头：CrowdStrike 以 34.4x 前瞻收入领跑安全（中位数 3.9x），Cloudflare 32.6x 对 17.5x，Shopify 11.3x 对 1.4x。
   [查看原文](https://www.tomtunguz.com/a-winner-in-every-category)

26. **Can you trust what AI tells you?** — Claude：YouTube（RSS）
   [查看原文](https://www.youtube.com/watch?v=cIMlBw2nqfA)
